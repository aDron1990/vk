...
void main()
{
    ...
    float aspect = textureSize(frameTexture, 0).x / textureSize(frameTexture, 0).y;
    vec3 result = texture(frameTexture, fragTexCoord).rgb * weight[0];

    for (int i = 1; i < 15; ++i) {
        result += texture(frameTexture, fragTexCoord + vec2(texOffset.x * i * aspect, 0.0)).rgb * weight[i];
        result += texture(frameTexture, fragTexCoord - vec2(texOffset.x * i * aspect, 0.0)).rgb * weight[i];
    }
